...
// @CO\tReporting AS: bitScore, ZR: rawScore, ZE: expected, ZI: percent identity, ZL: reference length, ZF: frame, ZS: query start DNA coordinate\n";
		// static const char* line = "AS: bitScore, ZR: rawScore, ZE: expected, ZI: percent identity, ZL: reference length, ZF: frame, ZS: query start DNA coordinate\n";
...
template<typename _val>
class PAF_format : public Output_format<_val>
{
	public:
	PAF_format()
	{ }
...
const Output_format<_val>& get_output_format()
{
	...
	static const PAF_format<_val> paf;
