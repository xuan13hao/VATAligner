...
struct shape
{
	...
	shape():
		...
		rev_mask_ (0),
		id_ (0)
	{ memset(positions_, 0, sizeof(uint32_t)*VATParameter::max_seed_weight); }
	...
	shape(const char *code, unsigned id):
		...
		id_ (id)
	{
		assert(id < VATParameter::max_shapes);
		...
		for(;i<strlen(code);++i) {
			rev_mask_ <<= 1;
			if(code[i] == '1') {
				assert(weight_ < VATParameter::max_seed_weight);
	...
	inline bool set_seed(uint64_t &s, const _val *seq) const
	{
		...
		for(unsigned i=0;i<weight_;++i) {
			...
			if(l == AlphabetFeature<_val>::mask_char || l == String_set<_val>::PADDING_CHAR)
				return false;
			l = mask_critical(l);
	...
	uint32_t length_, weight_, positions_[VATParameter::max_seed_weight], d_, mask_, rev_mask_, id_;
