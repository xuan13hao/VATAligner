...
void pairChimeraSeeds(std::vector<DiagonalSeeds> &seeds, 
std::vector<ChimeraAlnType> &paired_seeds,int q_len) {
    ...
    std::sort(seeds.begin(),seeds.end(),DiagonalSeeds::cmp_query_begin);
    int q_seq_len = q_len;
    ...
    {
        // cout<<seeds[j].qry_id<<"\t"<<seeds[j].sbj_id<<"\t"<<seeds[j].i<<"\t"<<seeds[j].j<<endl;
        int p = seeds[j].i;
        cand[p][cand_count[p] ++] = j;
        // cout<<j<<"\t"<<(cand_count[p])<<"\t"<<p<<endl;
    }
