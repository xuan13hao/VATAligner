...
int main(int ac, const char* av[])
{
	...
	try {
        ...
        aligner.add_options()
			...
			("match", po::value<bool>(&VATParameters::spilce)->default_value(5), "match score (5)")
			("mismatch", po::value<bool>(&VATParameters::chimera)->default_value(-4), "mismatch score (-4)")
			// ("whole-genome", po::value<bool>(&VATParameters::whole_genome)->default_value(0), "whole genome alignment (0)")
			("simd_sort", "SIMD-Sort Double Index")
