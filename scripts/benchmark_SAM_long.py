...
def parse_cigar(cigar_string):
    """
    Parse a CIGAR string to calculate reference start and end positions.

    Parameters:
        cigar_string (str): The CIGAR string from a SAM file.

    Returns:
        tuple: A tuple containing reference start and reference end positions.
    """
    reference_start = 0
    reference_end = 0
    current_position = 0

    # Regular expression pattern to split the CIGAR string
    ...
    for length, operation in cigar_pattern.findall(cigar_string):
        ...
        if operation in ['M', 'D', 'N', '=', 'X']:
            reference_end = current_position + length
    ...
    with open(sam_file, 'r') as file:
        for line in file:
            ...
            if len(fields) >= 11:
                ...
                s_start, s_end = parse_cigar(cigar)

                mappings.append({
                    'query_id': query_id,
                    'reference_id': reference_id,
                    's_start': position,
                    's_end': position + (s_end -s_start ),
                })
...
if __name__ == "__main__":
    ...
    # print(ground_truth)
    mappings = read_sam(blasttab_file)
    # print(mappings)
